            ...
            for product_cleaned_name, group_info in st.session_state.analysis_groups.items():
                ...
                if analysis_mode == "이번 거래 진단":
                    ...
                    if ts_res and not ts_res['all_trades'].empty:
                        ...
                        fig_ts.add_trace(go.Scatter(x=current_txs['Date'], y=current_txs['unitprice'], mode='markers', marker=dict(symbol='circle', color='rgba(0,0,0,0)', size=[s * 1.5 for s in current_bubble_sizes], line=dict(color='lightblue', width=4)), name='입력값', hovertemplate='<b>입력값</b><br>단가: $%{y:,.2f}<extra></extra>'))
